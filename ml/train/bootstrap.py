...
def install(repo_dir):
    run([sys.executable, "-m", "pip", "install", "--quiet", "-r", str(repo_dir / "ml/train/requirements.txt")])
    run([sys.executable, "-m", "pip", "install", "--quiet", "--no-deps", "-e", str(repo_dir / "ml")])
