...
PUBLISH_PATTERNS = ("*.log", "watch-report.txt", "release/*.json", "release/*.onnx", "runs/*.log",
...
def summarize(out_dir):
    ...
    for manifest_path in sorted((out_dir / "release").glob("*-manifest.json")):
        ...
        for name, entry in manifest.get("scan", {}).items():
            ...
            lines.append(f"{manifest['kind']:8s} {name:13s} {entry.get('verdict') or 'calibrated':10s} "
    ...
    for log_path in sorted(out_dir.glob("runs/*.log")) + sorted(out_dir.glob("*.log")):
